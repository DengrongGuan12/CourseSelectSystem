...
public class TraTeaXiuGaiMMPanel extends JPanel implements ActionListener, DocumentListener{
	 ...
	 * Create the panel.
	 */
	public TraTeaXiuGaiMMPanel(TrainingTeaBLService trainingTeaBLService, Message message) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(passc1.length!=passc2.length){
			JOptionPane.showMessageDialog(TrainingTeaWindow, "密码中存在空格，无法修改！");
			this.TrainingTeaWindow.stateNow.setText("修改失败！");
		}else if(passc1.length>100){
			JOptionPane.showMessageDialog(TrainingTeaWindow, "密码长度过长，无法修改！");
			this.TrainingTeaWindow.stateNow.setText("修改失败！");
		}else {
			try {
				message_ChangePassword = this.trainingTeaBLService.ChangePasswd(deanTeaID, password);
			} catch (Exception ex){
				ex.printStackTrace();
			}
			
			if (message_ChangePassword.getMesType().
					equals(MessageType.traTea_changePasswd_success)) { 
				this.message.getU().setPasswd(password);
				this.TrainingTeaWindow.stateNow.setText("修改成功！");
				...
				JOptionPane.showMessageDialog(TrainingTeaWindow, "你的网络貌似有点问题！");
				this.TrainingTeaWindow.stateNow.setText("修改失败！");
			}
		}
