...
public class ClassTeaXiuGaiMMPanel extends JPanel implements ActionListener, DocumentListener{
	...
	private ClassTeaBLService classTeaBLService;
	private ClassTeaWindow classTeaWindow;
	 ...
	 * Create the panel.
	 */
	public ClassTeaXiuGaiMMPanel(ClassTeaBLService classTeaBLService,Message message) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(passc1.length!=passc2.length){
			JOptionPane.showMessageDialog(classTeaWindow, "密码中存在空格，无法修改！");
			this.classTeaWindow.stateNow.setText("修改失败！");
		}else if(passc1.length>100){
			JOptionPane.showMessageDialog(classTeaWindow, "密码长度过长，无法修改！");
			this.classTeaWindow.stateNow.setText("修改失败！");
		}else {
			try {
				message_ChangePassword = this.classTeaBLService.ChangePasswd(deanTeaID, password);
			} catch (Exception ex){
				ex.printStackTrace();
			}
			
			if (message_ChangePassword.getMesType().
					equals(MessageType.tea_changePasswd_success)) { 
				this.message.getU().setPasswd(password);
				this.classTeaWindow.stateNow.setText("修改成功！");
				...
				JOptionPane.showMessageDialog(classTeaWindow, "你的网络貌似有点问题！");
				this.classTeaWindow.stateNow.setText("修改失败！");
			}
		}
