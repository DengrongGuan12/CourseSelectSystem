...
public class ChangePasswordPanel extends JPanel implements ActionListener, DocumentListener{
	 ...
	 * Create the panel.
	 */
	public ChangePasswordPanel(DeanTeaBLService deanTeaBLService, Message message) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(passc1.length!=passc2.length){
			JOptionPane.showMessageDialog(deanTeaWindow, "密码中存在空格，无法修改！");
			this.deanTeaWindow.stateNow.setText("修改失败！");
		}else if(passc1.length>100){
			JOptionPane.showMessageDialog(deanTeaWindow, "密码长度过长，无法修改！");
			this.deanTeaWindow.stateNow.setText("修改失败！");
		}else {
			try {
				message_ChangePassword = this.deanTeaBLService.ChangePasswd(deanTeaID, password);
			} catch (Exception ex){
				ex.printStackTrace();
			}
			
			if (message_ChangePassword.getMesType().
					equals(MessageType.deanTea_changePasswd_success)) { 
				this.message.getU().setPasswd(password);
				this.deanTeaWindow.stateNow.setText("修改成功！");
